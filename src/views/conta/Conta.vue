      ...
      <v-col>
        ...
        >
          <v-container>
            ...
            <v-row>
              <v-col>
                <span class="subtitle-1 white--text">Incluidas nas somas</span>
                <br />
                <span class="display-1 white--text">{{
                  $toReal(totalIncluidasSoma())
                }}</span
                ><br />
              </v-col>
            </v-row>
            <v-row>
              <v-col>
                <span class="title light-blue--text text--lighten-4">{{
                  $toReal(totalRestantes())
                }}</span
                ><br />
                <span class="blue--text light-blue--text text--darken-1">
                  Soma das demais contas</span
                >
              </v-col>
            </v-row>
          </v-container>
      ...
      <v-col>
        <v-card class="pa-sm-3 pa-md-7 pa-xs-0">
          <v-card-title class="pr-0">
            <v-icon x-large color="blue accent-2">mdi-bookmark</v-icon>Lista de
            contas
          </v-card-title>
          ...
          <v-card>
            <v-card-title primary-title>
              ...
              <v-btn
                ...
                small
                elevation="0"
                v-on:click="$root.$emit('conta-form::show', '')"
              >
            ...
            >
              <template v-slot:item.icone="{ item }">
                <v-avatar color="grey lighten-4">
                  <v-icon large :color="item.tipoContaCor"
                    >mdi-{{ item.tipoContaIcone }}</v-icon
                  >
                </v-avatar>
              </template>
...
export default {
  ...
  methods: {
    ...
    inativaConta(conta) {
      ...
      this.$root.$emit(
        "sweet-confirm::show",
        new Confirm("Deseja " + message + " a conta " + conta.nome + "?")
      );
      ...
      this.$root.$once("sweet-confirm::result", async (payload) => {
        if (payload) {
          ...
          try {
            ...
            this.$root.$emit(
              "sweet-alert::show",
              new Alert("Conta " + message + " com sucesso!", "success")
            );
          } catch (error) {
            ...
            this.$root.$emit(
              "sweet-alert::show",
              new Alert(
                error.body.message || "Houve um erro para" + message,
                "error",
                4000
              )
            );
    ...
    clickRow(e) {
      this.$router.push({ name: "conta-detalhe", params: { id: e.id } });
    },
  },
