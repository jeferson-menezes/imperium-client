      ...
      <v-col>
        <v-card class="pa-sm-3 pa-md-7 pa-xs-0">
          <v-card-title class="pr-0">
            <v-icon x-large color="red accent-2">mdi-bookmark</v-icon>Lista de
            despesas
          </v-card-title>
          <v-card>
            <v-card-title primary-title>
              ...
              <v-btn
                ...
                small
                elevation="0"
                v-on:click="$root.$emit('despesa-form::show', '')"
              >
            ...
            >
              ...
              <template v-slot:item.acoes="{ item }">
                <v-btn
                  ...
                  small
                  elevation="0"
                  @click="$root.$emit('despesa-form::show', item.id)"
                >
...
export default {
  ...
  methods: {
    ...
    listar() {
      if (this.filtro) {
        ...
        this.ActionListarDespesas(pageable).finally(
    ...
    filtrar() {
      ...
      if (Number(this.filtro.substr(0, 2))) {
        ...
        if (this.filtro.length === 7 && patternMes.test(this.filtro)) {
          ...
          this.ActionFiltrarPorMes(pageable).finally(
            () => (this.loadingData = false)
          );
        }
      } else {
      ...
      this.$root.$once("sweet-confirm::result", async (payload) => {
        if (payload) {
          try {
            ...
            this.$root.$emit(
              "sweet-alert::show",
              new Alert("Despesa excluida com sucesso!", "success")
            );
          } catch (error) {
            console.log(error);
            this.$root.$emit(
  ...
  created() {
    this.$root.$on("atualiza::despesas", () => {
      this.listar();
    });
  },
};
